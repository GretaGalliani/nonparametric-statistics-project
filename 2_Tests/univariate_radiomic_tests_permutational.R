...
p.val.ivm.pc = rep(0, p)

for (i in (1:p)){
  p.val.ivm.pc[i] <- perm_median_test(pc_ivm_t[,i],pc_ivm_f[,i])
}
...
length(which(p.val.ivm.pc<alpha)) # 13 significantly different variables 
...
for (i in (1:p)){
  p.val.ivm.pm[i] <- perm_median_test(pm_ivm_t[,i],pm_ivm_f[,i])
}
...
# Showing the results
which(p.val.ivm.pm<alpha) # 0 significantly different variables 
...
for (i in (1:p)){
  p.val.grad.pc[i] <- perm_median_test(pc_grad_t[,i],pc_grad_f[,i])
}
...
for (i in (1:p)){
  p.val.grad.pm[i] <- perm_median_test(pm_grad_t[,i],pm_grad_f[,i])
}
...
# Showing the results
which(p.val.grad.pm<alpha) # 0 significantly different variables 
...
for (i in (1:p)){
  p.val.da.pc[i] <- perm_median_test(pc_da_t[,i],pc_da_f[,i])
}
...
length(which(p.val.da.pc<alpha)) # 6 significantly different variables 
                                  # before outliers removal it was 7
...
for (i in (1:p)){
  p.val.da.pm[i] <- perm_median_test(pm_da_t[,i],pm_da_f[,i])
}
...
# Showing the results
which(p.val.da.pm<alpha) # 0 significantly different variables 
...
for (i in (1:p)){
  p.val.rel.pc[i] <- perm_median_test(pc_rel_t[,i],pc_rel_f[,i])
}
...
length(which(p.val.rel.pc<alpha)) # 1 significantly different variables 
                                  # before outliers removal it was 4
...
for (i in (1:p)){
  p.val.rel.pm[i] <- perm_median_test(pm_da_t[,i],pm_da_f[,i])
}
